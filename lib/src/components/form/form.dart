...
class FormController extends ChangeNotifier {
  ...
  void detach(FormKey key) {
    if (_attachedInputs.containsKey(key)) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        // FIXED: FormController.dispose() is called before the notification is sent
        if (_disposed) return;
        notifyListeners();
      });
    }
...
extension FormExtension on BuildContext {
  ...
  FutureOr<SubmissionResult> _chainedSubmitForm(Map<FormKey, Object?> values, Map<FormKey, ValidationResult> errors,
    ...
    if (value is Future<ValidationResult?>) {
      ...
      // FIXED: InvalidResult should be added to errors
      errors[entry.key] = value;
    }
    ...
    if (formController != null) {
      ...
      if (formEntry != null) {
        ...
        if (formKey.isInstanceOf(value) && formEntry._cachedValue != value) {
          ...
          return formController.attach(this, formKey, value, formEntry.widget.validator);
        }
      }
    }
    return null;
